...
function get_web_page($url, $agent = "desktop")
{
    ...
    $custom_headers = array('X-EC-COUNTRY: '.$id_country, 'X-EC-CURRENCY: '.$id_currency, 'X-EC-LANGUAGE: '.$id_lang);
    ...
    $options = array(
        ...
        CURLOPT_HTTPHEADER => $custom_headers
    );
...
$id_country = Tools::getValue('id_country');
$id_currency = Tools::getValue('id_currency');
$num_products = Tools::getValue('num_products');

if (!$id_lang || !$id_country || !$id_currency  || !$num_products) {
...
foreach ($product_to_cache as $id_product) {
    ...
    // var_dump($cat);
    if ($prod->active == '1') {
        $link = $context->link->getProductLink((int) $id_product, $prod->link_rewrite[$id_lang]);
        display($link);
        get_web_page($link);
        get_web_page($link, "mobile");
    }
